...
public class BoidManager : MonoBehaviour {
    ...
    void CreateBoid()
    {
        ...
        for (int i = 0; i < numberOfPredators; i++)
        {
            ...
            BoidAgents.Add(b);  // Adds newly created predator to the list
        }
    }
    ...
    private void UpdateBoids()
    {
        ...
        {
            if(g.GetComponent<Boid>().agressive != true)
            {
    ...
    private Vector3 Seperation(GameObject b)
    {
        ...
        foreach (GameObject g in BoidAgents)
        {
            if (g != b)
            {
                if( Vect3Dist(g.transform.position, b.transform.position) < 2 )
                {
                    r +=(b.transform.position - g.transform.position);
                }
            }
        }
        return r;
    ...
    private Vector3 Alignment(GameObject b)
    {
        ...
        return r - ((b.GetComponent<Boid>().velocity) / 10) ;
    }
